...
counties.sf <- st_read("./Data/Spatial/counties/cb_2018_us_county_20m.shp") %>% filter(STATEFP == 12) %>% filter(NAME == "Collier" | NAME == "Monroe" | NAME == "Miami-Dade")
```
...
```{r}
area <- counties.sf %>% group_by(STATEFP) %>% summarise()
area.utm <- st_transform(area,32617)
mangrove.utm <- st_transform(mangrove.sf,32617)
mangrove.area <- mangrove.utm[area.utm,]
mapview(mangrove.area)
