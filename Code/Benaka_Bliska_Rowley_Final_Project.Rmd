...
park.boundary <- st_read("./Data/Spatial/park/nps_boundary.shp") %>% filter(UNIT_NAME == "Everglades National Park") %>% select(UNIT_NAME, geometry)
...
mangrovemap <- mapview(mangrove.area, color = "green", col.regions = "dark green", alpha.regions = .4, layer.name = "Mangrove Habitat") + mapview(park.boundary, color = 'black', lwd = 2, col.regions = "black", alpha.regions = 0, layer.name = "Everglades National Park Boundary")+
...
treesSRS4$meanAGB <- na.approx(treesSRS4$meanAGB) #using`na.approx` fato fill in missing monthly data
