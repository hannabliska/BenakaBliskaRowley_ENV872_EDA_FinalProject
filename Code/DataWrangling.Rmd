...
```{r, wrangling nutrients data set for mean phosphorus and nitrogen}
...
```{r, adding site coordinates to the nutrients data set}
site.locations.sf <- st_as_sf(FCE.site.coordinates, coords=c('Longitude (Decimal Degrees)', 'Latitude (Decimal Degrees)'),crs=4326)
...
FCE.site.coordinates$`Latitude (Decimal Degrees)`<-as.float(FCE.site.coordinates$`Latitude (Decimal Degrees)`)
FCE.site.coordinates$`Longitude (Decimal Degrees)`<-as.float(FCE.site.coordinates$`Longitude (Decimal Degrees)`)

FCE.site.coordinates.sf <- st_as_sf(FCE.site.coordinates, coords=c(""))
...
tree.data.sf <- st_as_sf(FCE.tree.data, coords = c("Longitude (Decimal Degrees).y","Latitude (Decimal Degrees).y"), crs=4326) %>% 
  group_by(SITE, DATE) %>%
  summarize(
