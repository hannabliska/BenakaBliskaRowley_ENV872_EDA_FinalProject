...
inat.processed <- inat.all %>% 
  ...
  filter(scientific_name=="Felix concolor"|scientific_name=="Mephitis mephitis"| scientific_name=="Procyon lotor"|scientific_name=="Mustel vision"|scientific_name=="Lutra Canadensis"| scientific_name=="Lynx rufus"| scientific_name=="Odocoileus virginianus"| scientific_name=="Sylvilagus palustris"| scientific_name=="Sigmodon hispidus"| scientific_name=="Oryzomys palustris" | scientific_name == "Alligator mississippiensis" | scientific_name=="Anolis sagrei"|scientific_name=="Crocodylus acutus" | scientific_name == "Apalone ferox" | scientific_name=="Agkistrodon conanti"| scientific_name=="Psuedemys nelsoni"| scientific_name == "Python bivittatus" | scientific_name == "Anhinga anhinga"| scientific_name == "Ardea herodias"| scientific_name =="Pandion haliaetus"| scientific_name == "Ardea alba" | scientific_name == "Phalacrocorax auritus" | scientific_name == "Porphyrio martinica"| scientific_name == "Buteo lineatus" | scientific_name == "Eudocimus albus"| scientific_name =="Butorides virescens" | scientific_name == "Coragyps atratus"| scientific_name =="Mycteria americana" | scientific_name == "Picoides borealis"| scientific_name =="Rostrhamus socialbilis plumbeus" | scientific_name == "Sterna dougallii dougallii")
...
inat.spatial <- inat.processed %>% 
  st_as_sf(coords = c('longitude', 'latitude'), crs=32617)
inat.utm <- st_transform(inat.spatial, 32617)
...
R.trees <- merge(x=R.trees, y=FCE.site.coordinates, by="SITE", all.x = TRUE) %>%
  mutate(coordinates = paste(R.trees$`Longitude (Decimal Degrees)`, R.trees$`Latitude (Decimal Degrees)`, sep = ", ")) 

R.trees <- R.trees %>% select(DATE, SITE, Species.Tree, Tree_DBH, Tree_Height, meanWD, coordinates, "Latitude (Decimal Degrees).x", "Longitude (Decimal Degrees).x", geometry)


L.trees <- st_as_sf(FCE.tree.data.sites, coords = c("Longitude (Decimal Degrees)","Latitude (Decimal Degrees)"), crs=4326) %>%
  select(DATE, SITE, Species.Tree, Tree_DBH, Tree_Height, geometry) %>%
  filter(Species.Tree=="L") %>%
  mutate(getWoodDensity(genus = "Laguncularia", species = "racemosa"))

computeAGB(R.trees$Tree_DBH, R.trees$meanWD, coord = R.trees$coordinates)

computeAGB(R.trees$Tree_DBH, R.trees$meanWD, coord = cbind(as.numeric(R.trees$`Longitude (Decimal Degrees).x`, R.trees$`Latitude (Decimal Degrees).x`)))

computeAGB(R.trees$Tree_DBH, R.trees$meanWD, coord = c(as.numeric(R.trees$`Longitude (Decimal Degrees).x`, R.trees$`Latitude (Decimal Degrees).x`)))
