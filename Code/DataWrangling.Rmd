...
R.trees <- FCE.tree.data %>%
  select(DATE, SITE, Species.Tree, Tree_DBH, Tree_Height) %>%
  filter(Species.Tree=="R") %>%
  mutate(Genus = rep("Rhizophora"))%>%
  mutate(Species = rep("mangle"))

L.trees <- FCE.tree.data %>%
  select(DATE, SITE, Species.Tree, Tree_DBH, Tree_Height) %>%
  filter(Species.Tree=="L") %>%
  mutate(Genus = rep("Laguncularia")) %>%
  mutate(Species = rep("racemosa"))
