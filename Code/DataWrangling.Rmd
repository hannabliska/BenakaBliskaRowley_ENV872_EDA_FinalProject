...
R.trees <- merge(x=R.trees, y=FCE.site.coordinates, by="SITE", all.x = TRUE) %>%
  mutate(coordinates = paste(R.trees$`Longitude (Decimal Degrees).x`, R.trees$`Latitude (Decimal Degrees).x`, sep = ", ")) %>%
  select(DATE, SITE, Species.Tree, Tree_DBH, Tree_Height, meanWD, "Latitude (Decimal Degrees).x", "Longitude (Decimal Degrees).x", geometry)
...
getWoodDensity(genus = R.trees$Genus, species = R.trees$Species)
getWoodDensity(genu = L.trees$Genus, species = L.trees$Species)

computeAGB(R.trees$Tree_DBH, R.trees$meanWD, coord = paste(R.trees$`Longitude (Decimal Degrees).x`, R.trees$`Latitude (Decimal Degrees).x`, sep = " "))
